...
const NoteContextMenu: Component<{
  ...
  const noteContextForEveryone: () => MenuItem[] = () => {
    ...
    return [
      ...
      {
        label: [Kind.UserPoll, Kind.ZapPoll].includes(props.data?.note?.msg.kind) ? intl.formatMessage(tActions.pollContext.copyLink) : intl.formatMessage(tActions.noteContext.copyLink),
        action: copyNoteLink,
